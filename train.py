...
def train():
    ...
    users_count, repos_count = interaction_matrix.shape
    device = torch.device('cpu')
    train_graph.ndata['graph_data'] = {
        'user': torch.randn(users_count, 150, dtype=torch.float32, device=device), # 3 vectors with size 50
        'repo': torch.randn(repos_count, 361, dtype=torch.float32, device=device) # 7 vectors with size 50 + 6 number property + 5 boolean property
    }
    ...
    for epoch in range(EPOCH):
        ...
        if epoch % 5 == 0:
            ...
            for i, rating in enumerate(user_repo_rating):
                ...
                ground_truth = np.where(interaction_matrix[i]>0)[0]
